...
/* ── PARTICLE FIELD OLD (removed - redefined below) ── */
const ParticleField_OLD_REMOVE = ({count=60,color="#5b6cff"}) => {
...
/* ── 3D FLOATING PHONE OLD (removed - redefined below) ── */
const Phone3D_OLD_REMOVE = ({style={}}) => {
    ...
    <div ref={ref} style={{width:240,height:480,borderRadius:38,background:'linear-gradient(145deg,#0c0d1a,#05060f)',border:'2px solid rgba(91,108,255,.3)',boxShadow:'0 40px 120px rgba(91,108,255,.25),0 0 0 1px rgba(255,255,255,.05),inset 0 1px 0 rgba(255,255,255,.1)',position:'relative',overflow:'hidden',transformStyle:'preserve-3d',...style}}>
      ...
      <div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,#080912 0%,#02030a 100%)'}}>
        ...
        <div style={{padding:'8px 12px',display:'flex',flexDirection:'column',gap:6}}>
          {[['#FFE600','Impact',"YEAH CEST FOU 🔥"],['rgba(255,255,255,.9)','DM Sans',"attends de voir ca..."],['#34d399','DM Sans',"ca va tout changer"]].map(([color,font,text],i)=>(
...
/* ── AURORA BACKGROUND OLD (removed - redefined below) ── */
const AuroraBg_OLD_REMOVE = () => (
...
const MOCK_USERS=[
  {id:1,name:"Sophie Martin",email:"sophie@gmail.com",plan:"Pro",credits:Infinity,joined:"01/03/2026",status:"active",videos:47,country:"🇫🇷",avatar:"SM",lastSeen:"Il y a 2h"},
  {id:2,name:"Lucas Dubois",email:"lucas.dubois@outlook.com",plan:"Expert",credits:53,joined:"12/02/2026",status:"active",videos:23,country:"🇫🇷",avatar:"LD",lastSeen:"Il y a 1j"},
  {id:3,name:"Emma Wilson",email:"emma@icloud.com",plan:"Basic",credits:12,joined:"20/01/2026",status:"active",videos:8,country:"🇬🇧",avatar:"EW",lastSeen:"Il y a 3j"},
  {id:4,name:"Carlos García",email:"carlos@gmail.com",plan:"Free",credits:0,joined:"05/03/2026",status:"trial",videos:2,country:"🇪🇸",avatar:"CG",lastSeen:"Il y a 5min"},
  {id:5,name:"Yuki Tanaka",email:"yuki.t@yahoo.jp",plan:"Pro",credits:Infinity,joined:"28/01/2026",status:"active",videos:89,country:"🇯🇵",avatar:"YT",lastSeen:"En ligne"},
  {id:6,name:"Marie Lefebvre",email:"m.lefebvre@free.fr",plan:"Expert",credits:41,joined:"14/02/2026",status:"active",videos:31,country:"🇫🇷",avatar:"ML",lastSeen:"Il y a 4h"},
  {id:7,name:"James O'Brien",email:"james@gmail.com",plan:"Basic",credits:7,joined:"22/02/2026",status:"suspended",videos:5,country:"🇮🇪",avatar:"JO",lastSeen:"Il y a 8j"},
  {id:8,name:"Priya Sharma",email:"priya.s@gmail.com",plan:"Free",credits:3,joined:"06/03/2026",status:"trial",videos:1,country:"🇮🇳",avatar:"PS",lastSeen:"Il y a 10min"},
  {id:9,name:"Antoine Bernard",email:"a.bernard@gmail.com",plan:"Pro",credits:Infinity,joined:"08/01/2026",status:"active",videos:156,country:"🇫🇷",avatar:"AB",lastSeen:"Il y a 30min"},
  {id:10,name:"Lisa Chen",email:"lisa.chen@gmail.com",plan:"Expert",credits:28,joined:"19/02/2026",status:"active",videos:44,country:"🇨🇳",avatar:"LC",lastSeen:"Il y a 2j"},
  {id:11,name:"Mohamed Ali",email:"m.ali@hotmail.com",plan:"Basic",credits:19,joined:"25/01/2026",status:"active",videos:12,country:"🇸🇦",avatar:"MA",lastSeen:"Il y a 6h"},
  {id:12,name:"Clara Schmidt",email:"c.schmidt@gmail.de",plan:"Free",credits:2,joined:"03/03/2026",status:"trial",videos:3,country:"🇩🇪",avatar:"CS",lastSeen:"Il y a 1h"},
];

const MOCK_FILES=[
  {id:1,name:"Vlog Paris Ep.12",date:"07/03/2026",dur:"01:24",thumb:"🗼",exported:true,deleteIn:18,status:"ready",views:12400,style:"yellow-bold"},
  {id:2,name:"Tutorial React Hooks",date:"07/03/2026",dur:"00:53",thumb:"⚛️",exported:true,deleteIn:12,status:"ready",views:8200,style:"white-outline"},
  {id:3,name:"Recette pasta carbonara",date:"06/03/2026",dur:"00:47",thumb:"🍝",exported:false,deleteIn:6,status:"processing",views:0,style:"dark-card"},
  {id:4,name:"Gym motivation short",date:"06/03/2026",dur:"00:38",thumb:"💪",exported:true,deleteIn:3,status:"ready",views:31000,style:"neon-green"},
  {id:5,name:"Travel Barcelone",date:"05/03/2026",dur:"01:02",thumb:"🌴",exported:true,deleteIn:0,status:"deleted",views:5600,style:"pink-fire"},
];

const SUBTITLE_STYLES=[
  {id:"yellow-bold",label:"Yellow Bold",preview:{bg:"transparent",color:"#FFE600",font:"Impact",weight:900,size:68,shadow:"3px 3px 0 #000,-3px -3px 0 #000",transform:"uppercase"}},
  {id:"white-outline",label:"White Outline",preview:{bg:"transparent",color:"#fff",font:"Outfit",weight:800,size:60,shadow:"0 0 1px #000,2px 2px 0 #000,0 0 20px rgba(0,0,0,.8)",transform:"uppercase"}},
  {id:"blue-pop",label:"Blue Pop",preview:{bg:"transparent",color:"#4f6dff",font:"Outfit",weight:900,size:58,shadow:"2px 2px 0 #000",transform:"uppercase"}},
  {id:"dark-card",label:"Dark Card",preview:{bg:"rgba(0,0,0,.82)",color:"#fff",font:"Outfit",weight:700,size:52,shadow:"none",transform:"none"}},
  {id:"neon-green",label:"Neon Green",preview:{bg:"transparent",color:"#00e5a0",font:"Outfit",weight:700,size:56,shadow:"0 0 12px #00e5a0,0 0 30px #00e5a066",transform:"none"}},
  {id:"retro",label:"Retro",preview:{bg:"transparent",color:"#ffcc00",font:"Impact",weight:900,size:62,shadow:"4px 4px 0 #7c2d00",transform:"uppercase"}},
  {id:"pink-fire",label:"Pink Fire",preview:{bg:"transparent",color:"#ff4f8a",font:"Outfit",weight:800,size:60,shadow:"2px 2px 0 #000",transform:"none"}},
  {id:"minimal-white",label:"Minimal",preview:{bg:"transparent",color:"#fff",font:"Outfit",weight:400,size:46,shadow:"0 2px 8px rgba(0,0,0,.9)",transform:"none"}},
  {id:"purple-glow",label:"Purple Glow",preview:{bg:"transparent",color:"#9c6dff",font:"Outfit",weight:700,size:58,shadow:"0 0 16px #9c6dff88",transform:"none"}},
  {id:"red-urgent",label:"Red Urgent",preview:{bg:"rgba(220,38,38,.85)",color:"#fff",font:"Outfit",weight:800,size:52,shadow:"none",transform:"uppercase"}},

  // ── NOUVEAUX STYLES VIRAUX ──
  {id:"mrbeast",label:"MrBeast",creator:"MrBeast style",preview:{bg:"transparent",color:"#FFE600",font:"Impact",weight:900,size:72,shadow:"4px 4px 0 #000,-4px -4px 0 #000,4px -4px 0 #000,-4px 4px 0 #000,0 0 30px rgba(255,230,0,.6)",transform:"uppercase"}},
  {id:"squeezie",label:"Squeezie",creator:"Squeezie style",preview:{bg:"transparent",color:"#fff",font:"Outfit",weight:900,size:64,shadow:"0 0 2px #000,3px 3px 0 #4f6dff,-3px -3px 0 #4f6dff,3px -3px 0 #4f6dff,-3px 3px 0 #4f6dff",transform:"uppercase"}},
  {id:"lena-sit",label:"Léna Situations",creator:"Léna style",preview:{bg:"rgba(255,79,138,.92)",color:"#fff",font:"Outfit",weight:900,size:50,shadow:"none",transform:"none"}},
  {id:"double-outline",label:"Double Outline",creator:"Trending TikTok",preview:{bg:"transparent",color:"#fff",font:"Impact",weight:900,size:66,shadow:"3px 3px 0 #ff4f8a,-3px -3px 0 #ff4f8a,0 0 40px rgba(255,79,138,.5)",transform:"uppercase"}},
  {id:"fire-gradient",label:"Fire Gradient",creator:"Viral Reels",preview:{bg:"transparent",color:"#ff7043",font:"Outfit",weight:900,size:62,shadow:"0 0 20px #ff7043,0 0 40px #ff703399,3px 3px 0 #000",transform:"uppercase"}},
  {id:"cyan-punch",label:"Cyan Punch",creator:"Gaming/Esport",preview:{bg:"transparent",color:"#00d4ff",font:"Outfit",weight:900,size:62,shadow:"0 0 18px #00d4ff,0 0 36px #00d4ff66,2px 2px 0 #000",transform:"uppercase"}},
  {id:"black-yellow",label:"Black & Yellow",creator:"Hype/Rap",preview:{bg:"rgba(0,0,0,.9)",color:"#FFE600",font:"Impact",weight:900,size:58,shadow:"none",transform:"uppercase"}},
  {id:"stroke-white",label:"Stroke Clean",creator:"Instagram Clean",preview:{bg:"transparent",color:"#fff",font:"Outfit",weight:900,size:60,shadow:"-2px -2px 0 #000,2px -2px 0 #000,-2px 2px 0 #000,2px 2px 0 #000",transform:"none"}},
  {id:"horror",label:"Horror",creator:"Creepy/Thriller",preview:{bg:"transparent",color:"#ff3b5c",font:"Impact",weight:900,size:64,shadow:"0 0 20px #ff3b5c,0 0 60px #ff3b5c88,4px 0 0 #000,-4px 0 0 #000",transform:"uppercase"}},
  {id:"neon-blue-pulse",label:"Neon Blue",creator:"Cyber/Tech",preview:{bg:"transparent",color:"#4f6dff",font:"Outfit",weight:900,size:62,shadow:"0 0 10px #4f6dff,0 0 30px #4f6dff,0 0 60px #4f6dff66",transform:"uppercase"}},
  {id:"gold-luxury",label:"Gold Luxury",creator:"Business/Finance",preview:{bg:"transparent",color:"#ffd700",font:"Outfit",weight:900,size:58,shadow:"0 0 20px #ffd70099,2px 2px 0 #7c4f00",transform:"uppercase"}},
  {id:"white-blue-card",label:"White Card Blue",creator:"YouTube Trending",preview:{bg:"rgba(12,18,48,.93)",color:"#fff",font:"Outfit",weight:800,size:50,shadow:"none",transform:"none"}},
  {id:"pop-orange",label:"Pop Orange",creator:"TikTok Trend",preview:{bg:"transparent",color:"#ff7043",font:"Outfit",weight:900,size:64,shadow:"3px 3px 0 #000,-1px -1px 0 #000",transform:"uppercase"}},
  {id:"gradient-text",label:"Gradient Vibe",creator:"Instagram Aesthetic",preview:{bg:"transparent",color:"#9c6dff",font:"Outfit",weight:900,size:58,shadow:"0 0 30px #9c6dff66,2px 2px 0 #000",transform:"uppercase"}},
];

const ALL_DEMOS=[
  {id:"d1",type:"word-by-word",label:"Word by Word — Karaoke",color:"#FFE600",words:["Tu","vas","KIFFER","ce","truc","🔥"]},
  {id:"d2",type:"word-by-word",label:"Word by Word — Highlight",color:"#ff4f8a",words:["POV","t'es","en","train","de","changer","ta","VIE","💫"]},
  {id:"d3",type:"word-by-word",label:"Mot par Mot — Blanc",color:"#fff",words:["Regarde","ce","que","j'ai","trouvé","🤯"]},
  {id:"d4",type:"popup",label:"Popup — Yellow Bold",styleId:"yellow-bold",text:"REGARDE ÇA 😱"},
  {id:"d5",type:"popup",label:"Popup — White Outline",styleId:"white-outline",text:"C'EST INSANE 🚀"},
  {id:"d6",type:"popup",label:"Popup — Blue Pop",styleId:"blue-pop",text:"WAIT FOR IT..."},
  {id:"d7",type:"popup",label:"Popup — Neon Green",styleId:"neon-green",text:"VOILÀ LE SECRET ✨"},
  {id:"d8",type:"popup",label:"Popup — Pink Fire",styleId:"pink-fire",text:"C'est trop bien non?"},
  {id:"d9",type:"popup",label:"Popup — Purple Glow",styleId:"purple-glow",text:"Personne ne sait ça..."},
  {id:"d10",type:"popup",label:"Popup — Retro",styleId:"retro",text:"OLD SCHOOL VIBES 🎵"},
  {id:"d11",type:"line",label:"Dark Card — Multi lignes",styleId:"dark-card",text:"La vérité que personne\nte dit jamais 👀"},
  {id:"d12",type:"line",label:"Red Urgent",styleId:"red-urgent",text:"STOP! LIS ÇA"},
  {id:"d13",type:"line",label:"Minimal — Cinéma",styleId:"minimal-white",text:"Et c'est là que tout\na changé pour moi..."},
  {id:"d14",type:"shimmer",label:"Shimmer — Texte lumineux",text:"✨ INCROYABLE ✨"},
  {id:"d15",type:"typewriter",label:"Typewriter — Suspense",text:"Voilà comment gagner 10K€/mois..."},
  {id:"d16",type:"shake",label:"Shake — Vibration",text:"😱 NON NON NON 😱"},
  {id:"d17",type:"zoom-in",label:"Zoom In — Impact",text:"BOOM 💥"},
  {id:"d18",type:"bounce-words",label:"Bounce Words",text:"C'est • FOU • non? • 🤔"},
  {id:"d19",type:"emoji-line",label:"Full Emoji Line",text:"🔥💯🚀✨⚡🎯🏆💎"},
  {id:"d20",type:"popup",label:"Counted List",styleId:"yellow-bold",text:"#1 Conseil à retenir"},
  {id:"d21",type:"word-by-word",label:"Question Hook",color:"#00d4ff",words:["T'as","déjà","essayé","ça","?","🤔"]},
  {id:"d22",type:"popup",label:"CTA Direct",styleId:"white-outline",text:"LIKE SI T'ES D'ACCORD 👇"},
  {id:"d23",type:"line",label:"Stats Impact",styleId:"blue-pop",text:"97% des gens\nne savent pas ça"},
  {id:"d24",type:"popup",label:"Surprise Reveal",styleId:"neon-green",text:"Et voilà le résultat 😍"},
  {id:"d25",type:"typewriter",label:"Typewriter — Hook",text:"Ce que tu vas voir va te changer..."},
  {id:"d26",type:"word-by-word",label:"Emotional Hook",color:"#ff4f8a",words:["Je","n'aurais","JAMAIS","cru","ça","😭"]},
  {id:"d27",type:"shimmer",label:"Gold Shimmer",text:"💰 La méthode SECRÈTE 💰"},
  {id:"d28",type:"zoom-in",label:"Zoom — Chiffre clé",text:"1.000.000 vues 🎯"},
  {id:"d29",type:"bounce-words",label:"Rythme • Punchy",text:"Simple • Rapide • Efficace"},
  {id:"d30",type:"line",label:"Dark Card — Long",styleId:"dark-card",text:"Voici les 3 étapes\npour réussir en 2026"},
  {id:"d31",type:"popup",label:"MrBeast Style",styleId:"mrbeast",text:"IMPOSSIBLE DE CROIRE ÇA 😱"},
  {id:"d32",type:"popup",label:"Squeezie Style",styleId:"squeezie",text:"C'EST TROP FORT 🔥"},
  {id:"d33",type:"popup",label:"Léna Situations",styleId:"lena-sit",text:"ok mais SÉRIEUSEMENT 💅"},
  {id:"d34",type:"popup",label:"Double Outline",styleId:"double-outline",text:"REGARDE BIEN 👀"},
  {id:"d35",type:"popup",label:"Fire Gradient",styleId:"fire-gradient",text:"TOUT VA CHANGER 🔥"},
  {id:"d36",type:"popup",label:"Cyan Punch",styleId:"cyan-punch",text:"GG WP EZ CLAP 🎮"},
  {id:"d37",type:"popup",label:"Black & Yellow",styleId:"black-yellow",text:"LE VRAI SECRET 💛"},
  {id:"d38",type:"line",label:"Horror",styleId:"horror",text:"CE QUE TU VAS VOIR\nVA TE HANTER 😈"},
  {id:"d39",type:"popup",label:"Neon Blue Cyber",styleId:"neon-blue-pulse",text:"SYSTÈME ACTIVÉ ⚡"},
  {id:"d40",type:"popup",label:"Gold Luxury",styleId:"gold-luxury",text:"MÉTHODE À 10.000€/MOIS 💰"},
  {id:"d41",type:"popup",label:"Gradient Vibe",styleId:"gradient-text",text:"NOUVELLE ÈRE ✨"},
];
...
const PLANS=[
  {id:"free",name:"Free",price:0,priceY:0,color:T.muted,credits:3,features:["3 uploads / mois","Sous-titres auto","Templates tendance","Auto B-roll","Auto Zoom"],locked:["Téléchargement HD","Export vidéo","Traduction","API Access"]},
  {id:"basic",name:"Basic",price:12,priceY:10,color:T.cyan,credits:30,features:["30 shorts / mois","300 MB / vidéo","Polices custom","Full HD 60fps","Export Premiere / DaVinci","Support 24/7"],locked:[]},
  {id:"expert",name:"Expert",price:18,priceY:13,color:T.green,credits:100,features:["100 shorts / mois","400 MB / vidéo","Polices custom","Full HD 60fps","Export Premiere / DaVinci","Support 24/7"],locked:[]},
  {id:"pro",name:"Pro",price:30,priceY:21,color:T.acc,credits:Infinity,popular:true,features:["Illimité","600 MB / vidéo","Polices custom","Full HD 60fps","Export Premiere / DaVinci","Support prioritaire","Branding custom","API Access"],locked:[]},
];
...
/* ── PHONE MOCKUP ── */
const PhoneMockup=({subs,currentTime,styleId,fontSize,fontFamily,playing,onToggle})=>{
  ...
  const W=290; const H=520;
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12,flexShrink:0}}>
      <div style={{width:W,height:H,borderRadius:40,background:"#050810",border:"3px solid #252d45",boxShadow:"0 0 0 1px #1a2035,0 32px 64px rgba(0,0,0,.7),inset 0 1px 0 #3a4560",position:"relative",overflow:"hidden",flexShrink:0}}>
        <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:68,height:22,background:"#050810",borderRadius:"0 0 16px 16px",zIndex:20,border:"2px solid #252d45",borderTop:"none"}}/>
        <div style={{width:"100%",height:"100%",background:"linear-gradient(180deg,#141830 0%,#060810 60%,#0a0318 100%)",position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}>
          {/* Fake video scene */}
          <div style={{position:"absolute",inset:0,opacity:.12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:90}}>🎬</div>
          <div style={{position:"absolute",top:0,left:0,right:0,height:"55%",background:"linear-gradient(180deg,#1e2860 0%,transparent 100%)",opacity:.5}}/>
          <div style={{position:"absolute",bottom:0,left:0,right:0,height:"40%",background:"linear-gradient(0deg,#050215 0%,transparent 100%)",opacity:.7}}/>
          {/* Fake scene elements */}
          <div style={{position:"absolute",top:"20%",left:"15%",width:60,height:60,borderRadius:"50%",background:"radial-gradient(#4f6dff22,transparent)",animation:"breathe 3s ease infinite"}}/>
          <div style={{position:"absolute",top:"35%",right:"10%",width:40,height:40,borderRadius:"50%",background:"radial-gradient(#ff4f8a22,transparent)",animation:"breathe 4s ease infinite",animationDelay:".5s"}}/>
          {/* Subtitle overlay */}
          {active&&(
            <div key={active.id} style={{position:"absolute",bottom:"18%",left:0,right:0,textAlign:"center",padding:"0 12px",zIndex:10,animation:"subtitlePop .25s cubic-bezier(.34,1.56,.64,1) both"}}>
              <span style={{fontFamily:fontFamily||st.preview.font,fontSize:`${Math.round(st.preview.size*(W/380))}px`,fontWeight:st.preview.weight,color:st.preview.color,textShadow:st.preview.shadow,textTransform:st.preview.transform,background:st.preview.bg,padding:st.preview.bg!=="transparent"?"5px 12px":"0",borderRadius:"7px",display:"inline-block",lineHeight:1.25,whiteSpace:"pre-line",textAlign:"center"}}>
                {active.text}
              </span>
            </div>
          )}
          {/* Controls */}
          <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"8px 12px 14px",background:"linear-gradient(transparent,rgba(0,0,0,.7))"}}>
            <div style={{height:2,background:"rgba(255,255,255,.15)",borderRadius:2,marginBottom:8,position:"relative"}}>
              <div style={{height:"100%",width:`${Math.min((currentTime/totalDur)*100,100)}%`,background:T.acc,borderRadius:2,transition:"width .1s"}}/>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={onToggle} style={{width:26,height:26,borderRadius:"50%",background:"rgba(255,255,255,.18)",border:"none",color:"#fff",fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>{playing?"⏸":"▶"}</button>
              <span style={{fontSize:9,color:"rgba(255,255,255,.45)",fontFamily:"JetBrains Mono"}}>{currentTime.toFixed(1)}s / {totalDur.toFixed(0)}s</span>
              <div style={{marginLeft:"auto",display:"flex",gap:4}}>
                {["🔊","⛶"].map(ic=><div key={ic} style={{width:18,height:18,borderRadius:3,background:"rgba(255,255,255,.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8}}>{ic}</div>)}
              </div>
            </div>
          </div>
        </div>
        <div style={{position:"absolute",bottom:6,left:"50%",transform:"translateX(-50%)",width:70,height:3,background:"#2a3252",borderRadius:2}}/>
      </div>
      {/* Mini timeline */}
      <div style={{width:W,height:24,background:T.surf,borderRadius:8,border:`1px solid ${T.border}`,overflow:"hidden",position:"relative"}}>
        {subs.map(s=>(
          <div key={s.id} style={{position:"absolute",top:2,height:"calc(100% - 4px)",left:`${(s.start/totalDur)*100}%`,width:`${Math.max(((s.end-s.start)/totalDur)*100,.5)}%`,background:`${T.acc}35`,border:`1px solid ${T.acc}55`,borderRadius:2}}/>
        ))}
        <div style={{position:"absolute",top:0,bottom:0,left:`${Math.min((currentTime/totalDur)*100,100)}%`,width:2,background:T.pink,zIndex:3}}>
        ...
        <div key={step} style={{animation:"fadeUp .4s cubic-bezier(.22,1,.36,1)",background:T.surf,borderRadius:24,border:`1px solid ${T.border}`,padding:"36px 32px",boxShadow:"0 24px 80px rgba(0,0,0,.4)"}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:24,justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={()=>step<steps.length-1?setStep(s=>s+1):onComplete()} disabled={!canNext} className="btn-shimmer" style={{padding:"11px 26px",borderRadius:12,background:T.grad,border:"none",color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer",opacity:canNext?1:.45,transition:"all .2s"}}>
...
const LoginPage=({onAuth,onAdmin,goSignup})=>{
  ...
  const handle=()=>{
    setErr("");
    if(email==="admin@subcraftai.com"&&pw==="admin123"){onAdmin();return;}
          ...
          {PLANS.map((plan,i)=>(
            <div key={plan.id} style={{borderRadius:22,background:plan.popular?"linear-gradient(145deg,rgba(91,108,255,.1),rgba(168,85,247,.07))":T.surf,border:plan.popular?"1px solid rgba(91,108,255,.38)":"1px solid rgba(255,255,255,.055)",padding:"26px 20px",position:"relative",overflow:"hidden",transition:"all .3s",boxShadow:plan.popular?"0 0 60px rgba(91,108,255,.12)":"none",animation:`fadeUp .4s ${i*.07}s both`}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";}} onMouseLeave={e=>{e.currentTarget.style.transform="none";}}>
              ...
              <div style={{position:"relative",zIndex:1}}>
                ...
                {billingY&&plan.price>0&&<div style={{fontSize:10,color:T.muted,marginBottom:14}}>soit €{plan.priceY*12} facturé annuellement</div>}
...
const Dashboard=({user,setUser,onOpen,onLogout,setPage})=>{
  ...
  const [notifications,setNotifications]=useState([
    {id:1,icon:"✅",text:"Vlog Paris Ep.12 — Export terminé",time:"Il y a 2h",read:false},
    {id:2,icon:"⚠️",text:"Plus que 3 crédits restants ce mois",time:"Il y a 5h",read:false},
    {id:3,icon:"🎉",text:"Bienvenue sur SubCraft ! Explore tes outils.",time:"Il y a 1j",read:true},
    {id:4,icon:"🌍",text:"Traduction ES disponible pour Tutorial React",time:"Il y a 2j",read:true},
  ]);
  const unread=notifications.filter(n=>!n.read).length;
  const planColor={Free:T.muted,Basic:T.cyan,Expert:T.green,Pro:T.acc};
  const filtered=files.filter(f=>f.name.toLowerCase().includes(search.toLowerCase()));

  const handleUpload=(file,lang)=>{
    const newF={id:Date.now(),name:file?.name||"Nouvelle vidéo.mp4",date:new Date().toLocaleDateString("fr-FR"),dur:"00:00",thumb:"🎬",exported:false,deleteIn:24,status:"processing",views:0,style:"yellow-bold"};
    setFiles(p=>[newF,...p]);
    setShowUpload(false);
  ...
  const statsCards=[
    ...
    {icon:"💎",label:"Crédits restants",value:user?.plan==="Pro"?"∞":user?.plan==="Expert"?"41":"12",color:T.yellow},
    {icon:"⬆",label:"Exports réalisés",value:files.filter(f=>f.exported).length,color:T.green},
    {icon:"📅",label:"Membre depuis",value:"Mars 2026",color:T.cyan},
  ];

  const planCredits={Free:3,Basic:30,Expert:100,Pro:Infinity};
  const usedCredits=files.filter(f=>f.status==="ready").length;
  const totalCredits=planCredits[user?.plan]||3;
  const creditPct=totalCredits===Infinity?15:Math.round((usedCredits/totalCredits)*100);
  const creditWarn=totalCredits!==Infinity&&creditPct>=70;
  ...
  const sendChat=async()=>{
    ...
    try{
      ...
      setChatHistory(h=>[...h,{id:Date.now()+1,from:"support",text:"Désolée, je suis momentanément indisponible. Essaie de recharger ! 🔄",time:new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"}),read:false}]);
    }
    ...
    <div style={{minHeight:"100vh",background:T.bg,position:"relative"}} className="page"><AuroraBg subtle/><Cursor/>
      ...
      <nav style={{padding:"0 24px",borderBottom:"1px solid rgba(255,255,255,.05)",display:"flex",alignItems:"center",gap:10,background:"rgba(2,3,10,.88)",backdropFilter:"blur(32px)",height:62,position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",gap:2}} className="hide-mobile">
          {[["🎬","Mes vidéos",null],["🎨","Templates","templates"],["🎁","Parrainage","referral"],["💳","Abonnement","subscription"]].map(([ic,label,target])=>(
            <button key={label} onClick={()=>target&&setPage(target)} style={{padding:"6px 12px",borderRadius:9,background:"transparent",border:"none",color:T.muted,fontSize:12,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",gap:5,transition:"all .15s"}} onMouseEnter={e=>{e.currentTarget.style.background=`${T.acc}10`;e.currentTarget.style.color=T.text;}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color=T.muted;}}>
              {ic} {label}
            </button>
          ))}
        </div>
        <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}}>
          {/* Referral badge */}
          <button onClick={()=>setPage("referral")} className="hide-mobile" style={{display:"flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:100,background:`${T.green}10`,border:`1px solid ${T.green}25`,color:T.green,fontSize:11,fontWeight:700,cursor:"pointer",transition:"all .15s"}} onMouseEnter={e=>e.currentTarget.style.background=`${T.green}20`} onMouseLeave={e=>e.currentTarget.style.background=`${T.green}10`}>
            🎁 +1 mois gratuit si tu invites
          </button>
          {/* Notifications */}
          <div style={{position:"relative"}}>
            <button onClick={()=>setNotifPanel(p=>!p)} style={{width:34,height:34,borderRadius:9,background:notifPanel?`${T.acc}15`:T.surf,border:`1px solid ${notifPanel?T.acc:T.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,cursor:"pointer",transition:"all .2s"}}>🔔</button>
            {unread>0&&<div style={{position:"absolute",top:-3,right:-3,width:16,height:16,borderRadius:"50%",background:T.pink,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:800,color:"#fff"}}>{unread}</div>}
            {notifPanel&&(
              <div style={{position:"absolute",top:"calc(100% + 8px)",right:0,width:310,background:T.surf2,border:`1px solid ${T.border}`,borderRadius:14,boxShadow:"0 16px 48px rgba(0,0,0,.6)",zIndex:200,animation:"fadeDown .2s ease",overflow:"hidden"}}>
                <div style={{padding:"11px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <button onClick={()=>setNotifications(p=>p.map(n=>({...n,read:true})))} style={{background:"none",border:"none",color:T.acc,fontSize:11,cursor:"pointer",fontWeight:600}}>Tout lire</button>
                </div>
                {notifications.map(n=>(
                  <div key={n.id} onClick={()=>setNotifications(p=>p.map(x=>x.id===n.id?{...x,read:true}:x))} style={{padding:"10px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:9,cursor:"pointer",background:n.read?"transparent":`${T.acc}07`}}>
                    <span style={{fontSize:18,flexShrink:0}}>{n.icon}</span>
                    <div style={{flex:1}}>
                      <div style={{fontSize:11,fontWeight:n.read?400:600,lineHeight:1.4}}>{n.text}</div>
                      <div style={{fontSize:10,color:T.dim,marginTop:2}}>{n.time}</div>
                    </div>
                    {!n.read&&<div style={{width:6,height:6,borderRadius:"50%",background:T.acc,flexShrink:0,marginTop:5}}/>}
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Profile */}
          <button onClick={()=>setPage("profile")} style={{display:"flex",alignItems:"center",gap:7,padding:"4px 10px 4px 5px",borderRadius:10,background:T.surf,border:`1px solid ${T.border}`,cursor:"pointer",transition:"all .2s"}} onMouseEnter={e=>e.currentTarget.style.borderColor=T.acc} onMouseLeave={e=>e.currentTarget.style.borderColor=T.border}>
          ...
          <Btn size="sm" onClick={()=>setPage("pricing")} style={{background:T.grad,fontSize:12,padding:"6px 12px"}}>⭐ Passer Pro</Btn>
          <Btn v="ghost" size="sm" onClick={onLogout} className="hide-mobile" style={{fontSize:12}}>Déco</Btn>
        </div>
      </nav>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"28px 24px",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:22,flexWrap:"wrap",gap:12}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <Btn v="dark" size="sm" onClick={()=>setPage("templates")} icon="🎨">Templates</Btn>
            <Btn onClick={()=>setShowUpload(true)} icon="+" size="sm" style={{background:T.grad}}>Nouvelle vidéo</Btn>
          </div>
        </div>

        {/* ── STATS STRIP ── */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}} className="mobile-grid1">
          {[
            {icon:"🎬",label:"Vidéos prêtes",value:files.filter(f=>f.status==="ready").length,color:T.acc,sub:"ce mois"},
            {icon:"💎",label:"Crédits restants",value:totalCredits===Infinity?"∞":totalCredits-usedCredits,color:creditWarn?T.orange:T.yellow,sub:`/ ${totalCredits===Infinity?"∞":totalCredits}`},
        ...
        {/* ── CREDIT BAR ── */}
        <div style={{padding:"12px 16px",borderRadius:12,background:creditWarn?`${T.orange}08`:T.surf,border:`1px solid ${creditWarn?T.orange:T.border}`,marginBottom:12,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:180}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              <span style={{fontSize:11,fontWeight:700,color:creditWarn?T.orange:T.muted}}>💎 Utilisation crédits ce mois</span>
              <span style={{fontFamily:"JetBrains Mono",fontSize:11,fontWeight:700,color:creditWarn?T.orange:T.text}}>{usedCredits} / {totalCredits===Infinity?"∞":totalCredits}</span>
            </div>
            <div style={{height:6,background:T.border,borderRadius:3,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${Math.min(creditPct,100)}%`,background:creditWarn?`linear-gradient(90deg,${T.yellow},${T.orange})`:T.grad,borderRadius:3,transition:"width .7s",boxShadow:`0 0 8px ${creditWarn?T.orange:T.acc}55`}}/>
            </div>
          </div>
          {creditWarn&&user?.plan!=="Pro"&&<Btn size="sm" onClick={()=>setPage("pricing")} style={{background:T.grad,flexShrink:0,fontSize:11}}>⭐ Passer Pro</Btn>}
          {totalCredits===Infinity&&<Tag color={T.green} dot>Illimité ∞</Tag>}
        </div>

        {/* ── UPGRADE BANNER (free users) ── */}
        {user?.plan==="Free"&&(
          <div style={{padding:"13px 18px",borderRadius:12,background:`linear-gradient(135deg,${T.acc}10,${T.purple}06)`,border:`1px solid ${T.acc}28`,marginBottom:14,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
            <span style={{fontSize:22}}>🚀</span>
            <div style={{flex:1,minWidth:180}}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:2}}>Tu es sur le plan Free — 3 vidéos max</div>
              <div style={{color:T.muted,fontSize:11}}>Passe à Basic : 30 vidéos/mois, export HD, support prioritaire.</div>
            </div>
        ...
        <div style={{background:T.surf,borderRadius:18,border:"1px solid rgba(255,255,255,.055)",overflow:"hidden",boxShadow:"0 4px 40px rgba(0,0,0,.25)"}}>
          ...
          {tabFiles.length===0?(
            <div style={{textAlign:"center",padding:"52px 20px",color:T.muted}}>
              <div style={{fontSize:52,marginBottom:12,animation:"float 3s ease infinite"}}>🎬</div>
              <div style={{fontWeight:700,fontSize:16,color:T.text,marginBottom:8}}>Aucune vidéo ici</div>
              <div style={{fontSize:13,marginBottom:20}}>Importe ta première vidéo et regarde la magie opérer ✨</div>
              <Btn onClick={()=>setShowUpload(true)} style={{background:T.grad}} icon="+">Importer une vidéo</Btn>
            </div>
          ):(
            <div>
              {tabFiles.map((f,i)=>{
                ...
                return(
                  <div key={f.id} style={{display:"grid",gridTemplateColumns:"50px 1fr auto auto",alignItems:"center",gap:14,padding:"13px 18px",borderBottom:i<tabFiles.length-1?`1px solid ${T.border}`:"none",transition:"background .15s",cursor:"pointer",animation:`fadeUp .3s ease ${i*.04}s both`}}
                    onClick={()=>onOpen(f)}
                    onMouseEnter={e=>e.currentTarget.style.background=T.surf2}
                    onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    {/* Thumb */}
                    <div style={{width:46,height:38,borderRadius:9,background:`linear-gradient(135deg,${T.bg2},#0a0f1e)`,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{f.thumb}</div>
                    {/* Info */}
                    <div style={{minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:14,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginBottom:5}}>{f.name}</div>
                      <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={{fontSize:10,color:T.dim}}>{f.date}</span>
                        {f.dur&&f.dur!=="00:00"&&<span style={{fontSize:10,color:T.dim}}>· {f.dur}</span>}
                        {f.exported&&<Tag color={T.acc}>⬇ Exporté</Tag>}
                        <div style={{display:"flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:100,background:`${sc}12`,border:`1px solid ${sc}22`}}>
                          <span style={{width:5,height:5,borderRadius:"50%",background:sc,animation:f.status==="processing"?"pulseDot 1.5s infinite":undefined}}/>
                          <span style={{fontSize:10,color:sc,fontWeight:600}}>{f.status==="processing"?"En cours":f.status==="ready"?"Prêt":"Supprimée"}</span>
                        </div>
                      </div>
                    </div>
                    {/* Timer */}
                    <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
                      {f.deleteIn>0&&<DeleteTimer hours={f.deleteIn}/>}
                    </div>
                    {/* Actions */}
                    <div style={{display:"flex",gap:6,flexShrink:0}} onClick={e=>e.stopPropagation()}>
                      <button onClick={()=>onOpen(f)} style={{padding:"6px 14px",borderRadius:8,background:T.acc,border:"none",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s"}} onMouseEnter={e=>e.currentTarget.style.opacity=".85"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
                        ✏ Éditer
                      </button>
                      <button onClick={()=>{if(window.confirm("Supprimer ?"))setFiles(p=>p.filter(x=>x.id!==f.id));}} style={{width:32,height:32,borderRadius:8,background:`${T.pink}10`,border:`1px solid ${T.pink}25`,color:T.pink,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>🗑</button>
                    </div>
                  </div>
                );
              })}
...
const UploadModal=({onClose,onImport})=>{
  ...
  const doImport=()=>{
    ...
    const iv=setInterval(()=>{
      p+=Math.random()*18+5;
      if(p>=100){p=100;clearInterval(iv);setTimeout(()=>onImport(file,lang),600);}
      setProgress(Math.min(p,100));
    },100);
  };

  return(
    <Modal title="Importer une vidéo" subtitle="Shorts · Reels · TikToks — Max 300 MB" onClose={!uploading?onClose:undefined} width={520}>
      {!uploading&&(
      ...
      {uploading?(
        <div style={{textAlign:"center",padding:"24px 0"}}>
          ...
          <div style={{marginTop:18,fontSize:12,color:T.muted}}>
            {progress<30?"Lecture du fichier audio...":progress<60?"Transcription avec Whisper AI...":progress<85?"Génération des sous-titres...":"Finalisation..."}
          </div>
        </div>
      ):(
        <>
            ...
            <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);setFile(e.dataTransfer.files[0]);}} onClick={()=>fRef.current.click()} style={{border:`2px dashed ${drag||file?T.acc:T.border}`,borderRadius:14,padding:"30px 20px",textAlign:"center",cursor:"pointer",background:drag?`${T.acc}07`:"transparent",transition:"all .2s",marginBottom:18}}>
              ...
              {file?(
                <>
                  ...
                  <div style={{color:T.muted,fontSize:12}}>{(file.size/1024/1024).toFixed(1)} MB · ✓ Prête à importer</div>
                </>
              ):(
                <>
          ...
          {tab==="url"&&(
            <div style={{marginBottom:18}}>
              ...
              <div style={{position:"relative",marginBottom:8}}>
                <input value={urlVal} onChange={e=>{setUrlVal(e.target.value);checkUrl(e.target.value);}} placeholder="https://www.youtube.com/shorts/..." style={{width:"100%",padding:"11px 40px 11px 14px",borderRadius:10,fontSize:13,background:T.bg,border:`2px solid ${urlOk==="ok"?T.green:urlOk==="err"?T.pink:T.border}`,color:T.text,fontFamily:"JetBrains Mono",transition:"border .2s"}}/>
              ...
              return(
                <div key={p.id} style={{padding:"14px 18px",borderRadius:12,background:active?`${T.acc}08`:T.surf,border:`1px solid ${active?T.acc:T.border}`,display:"flex",alignItems:"center",gap:14,cursor:active?undefined:"pointer",transition:"all .15s"}}
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:T.muted}}>${p.priceY}/mois · {p.credits===Infinity?"Illimité":`${p.credits} crédits`}</div>
                  </div>
...
const EditorPage=({onBack,file})=>{
  ...
  const [styleId,setStyleId]=useState(file?.style||"yellow-bold");
  const [fontSize,setFontSize]=useState(68);
  const [maxChars,setMaxChars]=useState(12);
  const [fontFam,setFontFam]=useState("Impact");
  const [activeEff,setActiveEff]=useState(["Shadow","Double line"]);
  const [activeLineEff,setActiveLineEff]=useState([]);
  const [activeWordEff,setActiveWordEff]=useState(["Highlight"]);
  ...
  const addEmoji=async()=>{
    setAddingEmoji(true);
    try{
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,messages:[{role:"user",content:`Add 1 perfect emoji to each subtitle. Keep short. Reply ONLY lines, no numbers.\n\n${subs.map(s=>s.text).join("\n")}`}]})});
      const d=await r.json();
      const lines=d.content[0]?.text?.split("\n").filter(l=>l.trim())||[];
      saveSubs(subs.map((s,i)=>({...s,text:lines[i]||s.text})));
      notify("Emojis ajoutés par l'IA ! ✨","success");
    }catch{notify("Erreur API — vérifie ta clé","error");}
  ...
  const translate=async()=>{
    ...
    try{
      ...
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,messages:[{role:"user",content:`Translate to ${lang}. Keep emojis. Reply ONLY translated lines.\n\n${subs.map(s=>s.text).join("\n")}`}]})});
      const d=await r.json();
      const lines=d.content[0]?.text?.split("\n").filter(l=>l.trim())||[];
      saveSubs(subs.map((s,i)=>({...s,text:lines[i]||s.text})));
      setShowTrans(false);
      notify(`Traduit en ${lang} !`,"success");
    }catch{notify("Erreur traduction","error");}
  ...
  const EFF_ALL=["Double line","Use caps","Popup","Waves","Shake","Rotations","Shadow","Letter spacing","Glowy","Fade in","Fade out","Zoom","Background","Italic","Up"];
  ...
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:T.bg,overflow:"hidden"}}>
      ...
      {shortcut&&<div style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",padding:"8px 18px",borderRadius:10,background:T.surf3,border:`1px solid ${T.border}`,fontSize:13,fontWeight:600,zIndex:999,animation:"popIn .2s ease",pointerEvents:"none"}}>{shortcut}</div>}

      {/* Top bar */}
      <div style={{padding:"9px 18px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:8,background:T.bg2,flexShrink:0,flexWrap:"wrap"}}>
        <button onClick={onBack} style={{padding:"6px 14px",borderRadius:8,background:"transparent",border:`1px solid ${T.border}`,color:T.muted,fontSize:12,display:"flex",alignItems:"center",gap:5,cursor:"pointer"}}>← Retour</button>
        <div style={{width:1,height:18,background:T.border}}/>
        <span style={{fontSize:11,color:T.muted,fontFamily:"JetBrains Mono",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:200}}>{file?.name||"Nouvelle vidéo"}</span>
        <div style={{marginLeft:"auto",display:"flex",gap:6,flexWrap:"wrap"}}>
          <Tooltip text="Ctrl+Z"><button onClick={undo} disabled={!undoStack.length} style={{width:30,height:30,borderRadius:8,background:"transparent",border:`1px solid ${T.border}`,color:undoStack.length?T.text:T.dim,cursor:undoStack.length?"pointer":"not-allowed",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center"}}>↩</button></Tooltip>
          <Tooltip text="Ctrl+Y"><button onClick={redo} disabled={!redoStack.length} style={{width:30,height:30,borderRadius:8,background:"transparent",border:`1px solid ${T.border}`,color:redoStack.length?T.text:T.dim,cursor:redoStack.length?"pointer":"not-allowed",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center"}}>↪</button></Tooltip>
          <Btn v="dark" size="sm" onClick={()=>setShowTrans(true)} icon="🌍">Traduire</Btn>
          <Btn v="dark" size="sm" onClick={addEmoji} loading={addingEmoji} icon="😀">Emojis IA</Btn>
          <Btn size="sm" onClick={()=>setShowExport(true)} icon="⬆" style={{background:T.grad,animation:"glow 3s infinite"}}>Exporter</Btn>
        </div>
      </div>

      <div style={{flex:1,display:"grid",gridTemplateColumns:"auto 1fr",overflow:"hidden"}}>
        {/* Phone */}
        <div style={{padding:"20px 18px",display:"flex",flexDirection:"column",alignItems:"center",gap:14,borderRight:`1px solid ${T.border}`,background:T.bg,overflowY:"auto",minWidth:330}} className="hide-mobile">
          <PhoneMockup subs={subs} currentTime={ct} styleId={styleId} fontSize={fontSize} fontFamily={fontFam} playing={playing} onToggle={()=>setPlaying(p=>!p)}/>
          <div style={{display:"flex",gap:6}}>
            <Tooltip text="Space"><Btn v="dark" size="sm" onClick={()=>setPlaying(p=>!p)}>{playing?"⏸ Pause":"▶ Play"}</Btn></Tooltip>
            <Btn v="ghost" size="sm" onClick={()=>setCt(0)}>⏮</Btn>
          </div>
        </div>

        {/* Right panel */}
        <div style={{display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{display:"flex",borderBottom:`1px solid ${T.border}`,background:T.bg2,flexShrink:0,overflowX:"auto"}}>
            {TABS.map(t=>(
              <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"11px 14px",background:"transparent",border:"none",borderBottom:`2px solid ${tab===t.id?T.acc:"transparent"}`,color:tab===t.id?T.text:T.muted,fontSize:12,fontWeight:600,display:"flex",alignItems:"center",gap:5,whiteSpace:"nowrap",transition:"all .15s",cursor:"pointer"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:16}}>
            ...
            {tab==="captions"&&(
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <span style={{fontSize:12,color:T.muted,fontWeight:600}}>{subs.length} sous-titres</span>
                  <div style={{display:"flex",gap:6}}>
                ...
                {subs.map((sub,i)=>{
                  ...
                  return(
                    <div key={sub.id} style={{borderRadius:10,border:`1px solid ${isActive?T.acc:T.border}`,background:isActive?`${T.acc}0a`:T.surf,transition:"all .18s"}}>
                      <div style={{padding:"6px 10px",display:"flex",alignItems:"center",gap:8,borderBottom:`1px solid ${T.border}`}}>
                        <span style={{fontSize:9,color:T.dim,fontFamily:"JetBrains Mono",minWidth:90}}>{sub.start.toFixed(2)}s – {sub.end.toFixed(2)}s</span>
                        <div style={{display:"flex",gap:3,marginLeft:"auto"}}>
                          <button onClick={()=>{setEditId(sub.id);setEditTxt(sub.text);}} style={{padding:"2px 7px",borderRadius:5,background:"transparent",border:`1px solid ${T.border}`,color:T.muted,fontSize:10,cursor:"pointer"}}>✏️</button>
                          <button onClick={()=>saveSubs(subs.filter(s=>s.id!==sub.id))} style={{padding:"2px 6px",borderRadius:5,background:"transparent",border:"none",color:T.pink+"80",fontSize:11,cursor:"pointer"}}>✕</button>
                        </div>
                      </div>
                      <div style={{padding:"7px 10px"}}>
                        {isEditing?(
                          <div style={{display:"flex",gap:5}}>
                            <textarea value={editTxt} onChange={e=>setEditTxt(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();saveSubs(subs.map(s=>s.id===sub.id?{...s,text:editTxt}:s));setEditId(null);}if(e.key==="Escape")setEditId(null);}} style={{flex:1,padding:"6px 10px",borderRadius:8,fontSize:13,resize:"none",minHeight:44,lineHeight:1.5,background:T.surf3,border:`1px solid ${T.acc}`,color:T.text}} autoFocus/>
                            <div style={{display:"flex",flexDirection:"column",gap:3}}>
                              <button onClick={()=>{saveSubs(subs.map(s=>s.id===sub.id?{...s,text:editTxt}:s));setEditId(null);}} style={{padding:"4px 8px",borderRadius:6,background:`${T.green}18`,border:`1px solid ${T.green}33`,color:T.green,fontSize:11,cursor:"pointer"}}>✓</button>
                              <button onClick={()=>setEditId(null)} style={{padding:"4px 8px",borderRadius:6,background:"transparent",border:`1px solid ${T.border}`,color:T.muted,fontSize:11,cursor:"pointer"}}>✕</button>
                            </div>
                          </div>
                        ):(
                          <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                            {sub.text.split(" ").map((w,wi)=>(
                              <span key={wi} onClick={()=>{setEditId(sub.id);setEditTxt(sub.text);}} style={{padding:"2px 8px",borderRadius:5,background:T.surf3,border:`1px solid ${T.border}`,fontSize:13,fontWeight:500,cursor:"text",transition:"all .12s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=T.acc;e.currentTarget.style.color=T.acc;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color=T.text;}}>{w}</span>
                            ))}
                          </div>
                        )}
                      </div>
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {SUBTITLE_STYLES.map(s=>(
                    <div key={s.id} onClick={()=>{setStyleId(s.id);notify(`Style "${s.label}" appliqué !`,"success");}} style={{borderRadius:12,border:`2px solid ${styleId===s.id?T.acc:T.border}`,overflow:"hidden",cursor:"pointer",transition:"all .18s",transform:styleId===s.id?"scale(1.02)":"scale(1)"}}>
                      <div style={{height:80,background:"#040710",display:"flex",alignItems:"center",justifyContent:"center",padding:8,position:"relative"}}>
                        {styleId===s.id&&<div style={{position:"absolute",top:5,right:5,width:15,height:15,borderRadius:"50%",background:T.acc,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#fff"}}>✓</div>}
                        <span style={{fontFamily:s.preview.font,fontSize:`${Math.round(s.preview.size*.26)}px`,fontWeight:s.preview.weight,color:s.preview.color,textShadow:s.preview.shadow,textTransform:s.preview.transform,background:s.preview.bg,padding:s.preview.bg!=="transparent"?"3px 8px":"0",borderRadius:"4px",textAlign:"center",lineHeight:1.2}}>APERÇU</span>
                      </div>
                      <div style={{padding:"6px 10px",background:T.surf2,borderTop:`1px solid ${T.border}`}}>
                        <span style={{fontSize:11,fontWeight:700,color:styleId===s.id?T.acc:T.text}}>{s.label}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {tab==="edit"&&(
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                {[
                  {title:"📌 Apply on all",desc:"Effets appliqués sur tout le groupe",effs:EFF_ALL,active:activeEff,set:setActiveEff},
                  {title:"📄 Apply on lines",desc:"Effets ligne par ligne",effs:EFF_LINE,active:activeLineEff,set:setActiveLineEff},
                  {title:"🔤 Apply on words",desc:"Effets mot par mot",effs:EFF_WORD,active:activeWordEff,set:setActiveWordEff},
                ].map(g=>(
                  <div key={g.title} style={{background:T.surf,borderRadius:12,border:`1px solid ${T.border}`,padding:14}}>
                    <div style={{fontWeight:700,fontSize:13,marginBottom:2}}>{g.title}</div>
                    <div style={{fontSize:11,color:T.muted,marginBottom:10}}>{g.desc}</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                      {g.effs.map(eff=>{
                        const on=g.active.includes(eff);
                        return <button key={eff} onClick={()=>g.set(p=>on?p.filter(x=>x!==eff):[...p,eff])} style={{padding:"5px 11px",borderRadius:7,background:on?T.acc:"transparent",border:`1px solid ${on?T.acc:T.border}`,color:on?"#fff":T.muted,fontSize:11,fontWeight:on?600:400,transition:"all .12s",cursor:"pointer"}}>{eff}</button>;
                      })}
                    </div>
                  </div>
                ))}
            ...
            {tab==="config"&&(
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                <div style={{background:T.surf,borderRadius:12,border:`1px solid ${T.border}`,padding:16}}>
                  <div style={{fontWeight:700,fontSize:13,marginBottom:14}}>Templates sauvegardés</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8,marginBottom:12}}>
                    <input placeholder="Nom du template..." style={{padding:"9px 12px",borderRadius:9,fontSize:13,background:T.surf3,border:`1px solid ${T.border}`,color:T.text}}/>
                    <Btn v="dark" size="sm" onClick={()=>notify("Template sauvegardé !","success")}>Sauver</Btn>
                  </div>
                  <Select label="Charger un template" value="" onChange={()=>{}} options={[{value:"",label:"-- Sélectionner --"},...TEMPLATES.map(t=>({value:t.id,label:t.name}))]}/>
                </div>
                <div style={{background:T.surf,borderRadius:12,border:`1px solid ${T.border}`,padding:16}}>
                  <div style={{fontWeight:700,fontSize:13,marginBottom:16}}>Paramètres</div>
                  {[{label:`Max caractères : ${maxChars}`,v:maxChars,set:setMaxChars,min:4,max:30},{label:`Taille police : ${fontSize}px`,v:fontSize,set:setFontSize,min:28,max:120}].map(s=>(
                    <div key={s.label} style={{marginBottom:16}}>
                      <div style={{fontSize:13,fontWeight:500,marginBottom:9,color:T.text}}>{s.label}</div>
                      <input type="range" min={s.min} max={s.max} value={s.v} onChange={e=>s.set(+e.target.value)} style={{width:"100%",accentColor:T.acc}}/>
                    </div>
                  ))}
      ...
      {showExport&&(
        <Modal title="⬆ Exporter" subtitle={`${subs.length} sous-titres prêts à l'export`} onClose={()=>setShowExport(false)} width={420}>
          {[
            {icon:"🎬",title:"Vidéo avec sous-titres",desc:"MP4 avec sous-titres incrustés (FFmpeg backend requis)",action:()=>notify("Export vidéo disponible en production avec FFmpeg","info")},
            {icon:"📄",title:"Fichier .SRT",desc:"Format universel compatible tous lecteurs",action:exportSRT},
            {icon:"🎨",title:"Fichier .ASS",desc:"Avec styles et animations avancés",action:()=>{exportSRT();notify(".ASS exporté !","success");}},
      ...
      <div style={{maxWidth:1060,margin:"0 auto",position:"relative",zIndex:1}}>
        ...
        <div style={{textAlign:"center",marginBottom:48}}>
          <h1 style={{fontWeight:900,fontSize:"clamp(30px,5vw,54px)",letterSpacing:"-.03em",marginBottom:12}}>
            Start creating, <em style={{fontStyle:"italic",background:T.grad,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>today</em>.
          </h1>
          ...
          <div style={{display:"inline-flex",alignItems:"center",gap:16,padding:"12px 24px",borderRadius:100,background:T.surf,border:`1px solid ${T.border}`}}>
            ...
            <div>
              ...
              {yearly&&<div style={{fontSize:10,color:T.green,fontWeight:700}}>2 mois offerts 🎉</div>}
            </div>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:16}} className="mobile-grid1">
          {PLANS.map((plan,i)=>{
            const price=yearly?plan.priceY:plan.price;
            return(
              <div key={plan.id} style={{borderRadius:20,padding:"28px 24px",background:plan.popular?`linear-gradient(160deg,${T.acc}12,${T.surf})`:T.surf,border:`2px solid ${plan.popular?T.acc:T.border}`,position:"relative",transition:"transform .2s",animation:`fadeUp .4s ease ${i*.08}s both`,cursor:"pointer"}}
                onMouseEnter={e=>e.currentTarget.style.transform="translateY(-5px)"}
                onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
                {plan.popular&&<div style={{position:"absolute",top:-14,left:"50%",transform:"translateX(-50%)",padding:"5px 20px",borderRadius:100,background:T.acc,color:"#fff",fontSize:12,fontWeight:800,whiteSpace:"nowrap",boxShadow:`0 4px 20px ${T.accGlow}`}}>⭐ Le plus populaire</div>}
                <div style={{fontWeight:900,fontSize:18,marginBottom:3}}>{plan.name} <span style={{fontSize:14,fontWeight:400,color:T.muted}}>edition</span></div>
                <div style={{fontSize:12,color:T.muted,marginBottom:20}}>{plan.credits===Infinity?"Vidéos illimitées":`${plan.credits} vidéos/mois`}</div>
                <div style={{display:"flex",alignItems:"flex-end",gap:2,marginBottom:14}}>
                  <span style={{fontSize:18,color:T.muted,fontWeight:500}}>$</span>
                  <span style={{fontSize:52,fontWeight:900,lineHeight:1,color:plan.popular?T.acc:T.text}}>{price}</span>
                  <span style={{color:T.muted,fontSize:12,marginBottom:7}}>/Mo</span>
                </div>
                {yearly&&price>0&&<div style={{fontSize:11,color:T.green,fontWeight:700,marginBottom:14}}>💰 Économise ${plan.price*2}/an</div>}
                {price>0&&<Btn full v={plan.popular?"primary":"secondary"} onClick={()=>onSelect?.(plan.id)} style={{marginBottom:20}}>Start Now →</Btn>}
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {plan.features.map(f=><div key={f} style={{display:"flex",gap:8,fontSize:12,color:T.text,alignItems:"flex-start"}}><span style={{color:plan.popular?T.acc:T.green,flexShrink:0,marginTop:1}}>✓</span>{f}</div>)}
                  {plan.locked.map(f=><div key={f} style={{display:"flex",gap:8,fontSize:12,color:T.dim,alignItems:"flex-start"}}><span style={{flexShrink:0,marginTop:1}}>✗</span>{f}</div>)}
                </div>
              </div>
            );
          })}
        ...
        <div style={{maxWidth:660,margin:"60px auto 0"}}>
          ...
          {[
            ...
            ["Qu'est-ce qu'un crédit ?","Un crédit = une vidéo traitée. L'export du fichier SRT ne consomme pas de crédit."],
...
const AdminEmailTemplates=()=>{
  ...
  const [templates,setTemplates]=useState({
    ...
    upgrade:{subject:"⭐ Passe au niveau supérieur — Offre spéciale pour toi",preview:"Tu utilises presque tous tes crédits. C'est le moment de passer Pro.",trigger:"70% crédits utilisés",icon:"🚀",color:T.yellow},
...
Tu as **3 crédits gratuits** pour commencer. 
...
Tu es en train de cartonner sur SubCraft — tu as déjà utilisé **{{pct}}% de tes crédits** ce mois-ci !
...
const AdminEmailLogs=()=>{
  const MOCK_LOGS=[
    ...
    {id:3,type:"upgrade",icon:"🚀",to:"emma@icloud.com",name:"Emma Wilson",subject:"⭐ Passe au niveau supérieur — Offre spéciale",status:"delivered",time:"Auj. 11:04",trigger:"70% crédits",opened:false,clicked:false},
    ...
    {id:6,type:"upgrade",icon:"🚀",to:"m.lefebvre@free.fr",name:"Marie Lefebvre",subject:"⭐ Passe au niveau supérieur — Offre spéciale",status:"bounced",time:"Hier 20:33",trigger:"70% crédits",opened:false,clicked:false},
    ...
    {id:10,type:"upgrade",icon:"🚀",to:"lisa.chen@gmail.com",name:"Lisa Chen",subject:"⭐ Passe au niveau supérieur",status:"delivered",time:"07/03 08:02",trigger:"70% crédits",opened:true,clicked:true},
  ...
  const SERVICES=[
    ...
    },
    {
      ...
      steps:[
        ...
        {n:4,title:"Configurer le webhook",desc:"Developers → Webhooks → Add endpoint. URL: https://ton-site.com/api/stripe-webhook. Événements: checkout.session.completed, customer.subscription.deleted."},
...
const AdminLandingEditor=()=>{
  const [heroTitle,setHeroTitle]=useState("Des sous-titres");
  const [heroTitle2,setHeroTitle2]=useState("qui font virer");
  const [heroTitle3,setHeroTitle3]=useState("viral 🚀");
  ...
  const [ctaText,setCtaText]=useState("Générer mes sous-titres — Gratuit");
  const [phoneSubtitle1,setPhoneSubtitle1]=useState("YEAH CEST FOU 🔥");
  const [phoneSubtitle2,setPhoneSubtitle2]=useState("attends de voir la suite...");
  const [phoneSubtitle3,setPhoneSubtitle3]=useState("ça va tout changer 🚀");
  const [phoneSub1Color,setPhoneSub1Color]=useState("#FFE600");
  const [phoneSub2Color,setPhoneSub2Color]=useState("rgba(255,255,255,.9)");
  const [phoneSub3Color,setPhoneSub3Color]=useState("#34d399");
  const [phoneSub1Font,setPhoneSub1Font]=useState("Impact");
  const [phoneBgText,setPhoneBgText]=useState("REGARDE ÇA 😱");
  const [saved,setSaved]=useState(false);

  const save=()=>{
    setSaved(true);
    notify("✅ Landing page mise à jour !","success");
    setTimeout(()=>setSaved(false),2500);
  };

  const ColorPick=({label,value,onChange})=>(
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      <label style={{fontSize:12,color:T.muted,minWidth:80}}>{label}</label>
      ...
      <input value={value} onChange={e=>onChange(e.target.value)}
        style={{flex:1,padding:"6px 10px",borderRadius:8,fontSize:12,background:T.bg,border:`1px solid ${T.border}`,color:T.text,fontFamily:"JetBrains Mono"}}/>
    </div>
  );

  // Live phone preview
  const PhonePreview=()=>(
    <div style={{width:200,height:400,borderRadius:32,background:"linear-gradient(145deg,#0c0d1a,#05060f)",border:"2px solid rgba(91,108,255,.3)",boxShadow:"0 20px 60px rgba(91,108,255,.2)",position:"relative",overflow:"hidden",flexShrink:0}}>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(180deg,#141830 0%,#020310 100%)"}}>
        <div style={{display:"flex",justifyContent:"space-between",padding:"10px 16px 0",fontSize:9,color:"rgba(255,255,255,.4)"}}>
          <span>9:41</span><span>●●●●</span>
        </div>
        <div style={{width:60,height:14,background:"#02030a",borderRadius:10,margin:"0 auto",marginTop:-2}}/>
        <div style={{margin:"12px 10px",height:160,borderRadius:12,background:"linear-gradient(135deg,#0d0e20,#1a0a2e)",overflow:"hidden",position:"relative"}}>
          <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(91,108,255,.2),rgba(168,85,247,.15))"}}/>
          <div style={{position:"absolute",bottom:12,left:0,right:0,textAlign:"center"}}>
            <div style={{display:"inline-block",background:"rgba(0,0,0,.7)",backdropFilter:"blur(8px)",borderRadius:6,padding:"3px 10px",fontFamily:"Impact",fontSize:14,color:"#FFE600",textShadow:"1px 1px 0 #000",letterSpacing:1}}>{phoneBgText}</div>
          </div>
        </div>
        <div style={{display:"flex",gap:2,alignItems:"center",justifyContent:"center",padding:"6px 16px"}}>
          {Array.from({length:20},(_,i)=>(
            <div key={i} style={{width:2,borderRadius:2,background:`rgba(91,108,255,${.3+Math.abs(Math.sin(i*.6))*.7})`,height:(6+Math.abs(Math.sin(i*.8))*14)+"px"}}/>
          ))}
        </div>
        <div style={{padding:"6px 10px",display:"flex",flexDirection:"column",gap:4}}>
          {[[phoneSub1Color,phoneSub1Font,phoneSubtitle1],[phoneSub2Color,"DM Sans",phoneSubtitle2],[phoneSub3Color,"DM Sans",phoneSubtitle3]].map(([color,font,text],i)=>(
            <div key={i} style={{padding:"3px 8px",borderRadius:5,background:i===0?"transparent":"rgba(0,0,0,.5)",textAlign:"center",fontFamily:font,fontSize:9,fontWeight:i===0?900:600,color,textShadow:i===0?"1px 1px 0 #000":undefined}}>{text}</div>
          ))}
        </div>
      </div>
    </div>
  ...
  return(
    <div className="page" style={{maxWidth:1100}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28}}>
        <div>
          <h1 style={{fontWeight:800,fontSize:22,marginBottom:4}}>🖥️ Éditeur Landing Page</h1>
          <div style={{color:T.muted,fontSize:13}}>Modifie le contenu de ta page d'accueil en temps réel</div>
        </div>
        <Btn onClick={save} loading={saved} style={{background:T.grad}} icon="💾">{saved?"Sauvegardé !":"Sauvegarder"}</Btn>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 220px",gap:24,alignItems:"start"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          {/* Hero Section */}
          <div style={{background:T.surf,borderRadius:16,border:`1px solid ${T.border}`,padding:20}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:18}}>🎯</span> Section Hero
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <Input label="Titre ligne 1" value={heroTitle} onChange={setHeroTitle} placeholder="Des sous-titres"/>
              <Input label="Titre ligne 2 (gradient)" value={heroTitle2} onChange={setHeroTitle2} placeholder="qui font virer"/>
              <Input label="Titre ligne 3" value={heroTitle3} onChange={setHeroTitle3} placeholder="viral 🚀"/>
              <div>
                <label style={{fontSize:12,color:T.muted,display:"block",marginBottom:6,fontWeight:600}}>Description</label>
                <textarea value={heroDesc} onChange={e=>setHeroDesc(e.target.value)} rows={3}
                  style={{width:"100%",padding:"10px 14px",borderRadius:10,fontSize:13,background:T.bg,border:`1px solid ${T.border}`,color:T.text,resize:"vertical",lineHeight:1.6}}/>
              </div>
              <Input label="Texte CTA principal" value={ctaText} onChange={setCtaText} icon="✦"/>
            </div>
          </div>
          ...
          <div style={{background:T.surf,borderRadius:16,border:`1px solid ${T.border}`,padding:20}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:18}}>📱</span> Téléphone 3D — Sous-titres
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <Input label="Texte vidéo (fond)" value={phoneBgText} onChange={setPhoneBgText} placeholder="REGARDE ÇA 😱"/>
              <div style={{height:1,background:T.border}}/>
              <div style={{fontWeight:600,fontSize:13,color:T.muted}}>Sous-titre 1 (style MrBeast)</div>
              <Input label="Texte" value={phoneSubtitle1} onChange={setPhoneSubtitle1}/>
              <ColorPick label="Couleur" value={phoneSub1Color} onChange={setPhoneSub1Color}/>
              <div>
                <label style={{fontSize:12,color:T.muted,display:"block",marginBottom:6,fontWeight:600}}>Police</label>
                <select value={phoneSub1Font} onChange={e=>setPhoneSub1Font(e.target.value)}
                  style={{width:"100%",padding:"8px 12px",borderRadius:8,background:T.bg,border:`1px solid ${T.border}`,color:T.text,fontSize:13}}>
                  {["Impact","DM Sans","Syne","Bebas Neue","Montserrat","Oswald"].map(f=><option key={f}>{f}</option>)}
                </select>
              </div>
              <div style={{height:1,background:T.border}}/>
              <div style={{fontWeight:600,fontSize:13,color:T.muted}}>Sous-titre 2</div>
              <Input label="Texte" value={phoneSubtitle2} onChange={setPhoneSubtitle2}/>
              <ColorPick label="Couleur" value={phoneSub2Color} onChange={setPhoneSub2Color}/>
              <div style={{height:1,background:T.border}}/>
              <div style={{fontWeight:600,fontSize:13,color:T.muted}}>Sous-titre 3</div>
              <Input label="Texte" value={phoneSubtitle3} onChange={setPhoneSubtitle3}/>
              <ColorPick label="Couleur" value={phoneSub3Color} onChange={setPhoneSub3Color}/>
            </div>
          </div>
          ...
          <div style={{background:T.surf,borderRadius:16,border:`1px solid ${T.border}`,padding:20}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:18}}>📊</span> Chiffres clés (barre hero)
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {[["Créateurs actifs","10k+"],["Précision Whisper","99%"],["Génération","< 10s"],["Styles viraux","24"]].map(([label,val])=>(
                <div key={label} style={{padding:"10px 12px",borderRadius:10,background:T.bg,border:`1px solid ${T.border}`}}>
                  <div style={{fontSize:11,color:T.muted,marginBottom:4}}>{label}</div>
                  <input defaultValue={val} style={{width:"100%",background:"none",border:"none",color:T.acc,fontWeight:800,fontSize:16,fontFamily:"JetBrains Mono"}}/>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Right: Live preview */}
        <div style={{position:"sticky",top:20,display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
          <div style={{fontSize:11,color:T.muted,fontWeight:700,letterSpacing:".06em",textTransform:"uppercase"}}>Aperçu live</div>
          <PhonePreview/>
          <div style={{background:T.surf,borderRadius:12,border:`1px solid ${T.border}`,padding:14,width:"100%",fontSize:11}}>
            <div style={{color:T.muted,marginBottom:8}}>Titre hero :</div>
            <div style={{fontWeight:800,fontSize:13,lineHeight:1.4}}>{heroTitle}<br/><span style={{background:"linear-gradient(135deg,#5b6cff,#a855f7)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{heroTitle2}</span><br/>{heroTitle3}</div>
          </div>
          <div style={{padding:"10px 14px",borderRadius:10,background:`${T.green}10`,border:`1px solid ${T.green}25`,width:"100%",fontSize:11,color:T.green,textAlign:"center"}}>
...
const AdminPanel=({onExit})=>{
  ...
  const totalMRR=users.reduce((a,u)=>a+({Free:0,Basic:10,Expert:13,Pro:21}[u.plan]||0),0);
  ...
  const NAV=[
    ...
    {id:"landing-editor",icon:"🖥️",label:"Éditeur Landing"},
    {id:"analytics",icon:"📈",label:"Statistiques"},
    {id:"analytics-adv",icon:"🔬",label:"Analytics Avancés"},
    {id:"revenue",icon:"💰",label:"Revenus"},
    {id:"tickets",icon:"🎫",label:"Tickets Support"},
    {id:"users",icon:"👥",label:"Utilisateurs"},
    {id:"leaderboard",icon:"🏆",label:"Leaderboard"},
    {id:"search",icon:"🔍",label:"Recherche Globale"},
    {id:"coupons",icon:"🎟️",label:"Coupons"},
    {id:"marketing",icon:"📣",label:"Marketing"},
    {id:"seo",icon:"🌍",label:"SEO & Contenu"},
    {id:"live-chat",icon:"💬",label:"Live Chat"},
    {id:"reports",icon:"📋",label:"Rapports Auto"},
    {id:"flags",icon:"🚦",label:"Feature Flags"},
    {id:"customize",icon:"🎨",label:"Personnalisation"},
    {id:"email-templates",icon:"📧",label:"Templates emails"},
    {id:"email-logs",icon:"📬",label:"Logs emails"},
    {id:"queue",icon:"⚡",label:"File de traitement"},
    {id:"system-logs",icon:"🖥️",label:"Logs Système"},
    {id:"api-keys",icon:"🔑",label:"Clés API"},
    {id:"storage",icon:"💾",label:"Stockage & Infra"},
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",background:T.bg}}>
      {/* Sidebar */}
      <div style={{width:220,background:T.bg2,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",padding:18,flexShrink:0,position:"sticky",top:0,height:"100vh"}} className="hide-mobile">
        <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:28}}>
          <div style={{width:30,height:30,borderRadius:8,background:T.gradP,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>⚡</div>
          <div>
            <div style={{fontWeight:800,fontSize:14}}>Admin Panel</div>
            <div style={{fontSize:10,color:T.muted}}>SubCraft v4</div>
          </div>
        </div>
        {NAV.map(n=>(
          <button key={n.id} onClick={()=>setView(n.id)} style={{padding:"9px 12px",borderRadius:10,background:view===n.id?`${T.acc}15`:"transparent",border:"none",color:view===n.id?T.text:T.muted,fontSize:13,fontWeight:view===n.id?700:400,cursor:"pointer",display:"flex",alignItems:"center",gap:9,marginBottom:3,textAlign:"left",width:"100%",transition:"all .15s"}}
            onMouseEnter={e=>{if(view!==n.id)e.currentTarget.style.background=`${T.acc}08`;}}
        ...
        <div style={{marginTop:"auto"}}>
          <Divider/>
          <Btn v="danger" onClick={onExit} full size="sm">← Quitter Admin</Btn>
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:28}}>
        ...
        {view==="dashboard"&&(
          <div className="page">
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:12,marginBottom:28}} className="mobile-grid1">
              {[
                {icon:"👥",label:"Total users",value:users.length,color:T.acc,sub:"+3 ce mois"},
                {icon:"💰",label:"MRR estimé",value:`$${totalMRR}`,color:T.green,sub:"Mensuel récurrent"},
                {icon:"⭐",label:"Plan Pro",value:users.filter(u=>u.plan==="Pro").length,color:T.yellow,sub:`${Math.round(users.filter(u=>u.plan==="Pro").length/users.length*100)}% des users`},
                {icon:"🎬",label:"Vidéos traitées",value:users.reduce((a,u)=>a+u.videos,0),color:T.cyan,sub:"Total"},
                {icon:"🔴",label:"Suspendus",value:users.filter(u=>u.status==="suspended").length,color:T.pink,sub:"À traiter"},
                {icon:"🆕",label:"Trials actifs",value:users.filter(u=>u.status==="trial").length,color:T.purple,sub:"Conversion possible"},
              ].map(s=>(
                <div key={s.label} style={{padding:"18px 18px",borderRadius:12,background:T.surf,border:`1px solid ${T.border}`,position:"relative",overflow:"hidden"}}>
                  <div style={{position:"absolute",top:-10,right:-10,width:50,height:50,borderRadius:"50%",background:`${s.color}12`}}/>
                  <div style={{fontSize:22,marginBottom:7}}>{s.icon}</div>
                  <div style={{fontFamily:"JetBrains Mono",fontWeight:700,fontSize:24,color:s.color}}>{s.value}</div>
                  <div style={{fontSize:11,color:T.muted,marginTop:3}}>{s.label}</div>
                  {s.sub&&<div style={{fontSize:10,color:T.green,marginTop:2}}>{s.sub}</div>}
                </div>
              ))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}} className="mobile-grid1">
              <div style={{background:T.surf,borderRadius:14,border:`1px solid ${T.border}`,padding:18}}>
                <div style={{fontWeight:700,fontSize:14,marginBottom:16}}>Répartition des plans</div>
                {["Free","Basic","Expert","Pro"].map(p=>{
                  ...
                  return(
                    <div key={p} style={{marginBottom:12}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                        <span style={{fontSize:12,fontWeight:600,color:planColor[p]||T.muted}}>{p}</span>
                        <span style={{fontSize:11,color:T.muted,fontFamily:"JetBrains Mono"}}>{c} users · {pct}%</span>
                      </div>
                      <div style={{height:5,background:T.border,borderRadius:3,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${pct}%`,background:planColor[p]||T.muted,borderRadius:3,transition:"width .6s"}}/>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{background:T.surf,borderRadius:14,border:`1px solid ${T.border}`,padding:18}}>
                <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>Activité récente</div>
                {users.slice(0,6).map(u=>(
                  <div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:`1px solid ${T.border}`}}>
                    <Avatar name={u.name} size={26}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{u.country} {u.name}</div>
                      <div style={{fontSize:10,color:T.muted}}>{u.lastSeen}</div>
                    </div>
                    <Tag color={planColor[u.plan]||T.muted}>{u.plan}</Tag>
                  </div>
                ))}
              </div>
            </div>
        ...
        {view==="users"&&(
          <div className="page">
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:22,flexWrap:"wrap",gap:12}}>
              <div>
                <h1 style={{fontWeight:800,fontSize:22,marginBottom:4}}>👥 Utilisateurs</h1>
                <div style={{color:T.muted,fontSize:13}}>{filtered.length}/{users.length} utilisateurs</div>
              </div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <div style={{position:"relative"}}>
                  <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",fontSize:12}}>🔍</span>
                  <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher..." style={{padding:"8px 12px 8px 30px",borderRadius:10,fontSize:13,background:T.surf,border:`1px solid ${T.border}`,color:T.text,width:200}}/>
                </div>
                <select value={filterPlan} onChange={e=>setFilterPlan(e.target.value)} style={{padding:"8px 12px",borderRadius:10,fontSize:13,background:T.surf,border:`1px solid ${T.border}`,color:T.text}}>
                  <option value="all">Tous les plans</option>
                  {["Free","Basic","Expert","Pro"].map(p=><option key={p}>{p}</option>)}
                </select>
                <Btn size="sm" onClick={()=>setShowAdd(true)} icon="➕">Ajouter</Btn>
              </div>
            </div>
            <div style={{background:T.surf,borderRadius:14,border:`1px solid ${T.border}`,overflow:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:"28px 1fr 180px 90px 80px 80px 90px 120px",padding:"10px 16px",borderBottom:`1px solid ${T.border}`,fontSize:10,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:".07em",gap:8}}>
                <span>#</span><span>Utilisateur</span><span>Email</span><span>Plan</span><span>Crédits</span><span>Vidéos</span><span>Statut</span><span>Actions</span>
              </div>
              {filtered.map((u,i)=>(
                <div key={u.id} style={{display:"grid",gridTemplateColumns:"28px 1fr 180px 90px 80px 80px 90px 120px",alignItems:"center",padding:"11px 16px",borderBottom:i<filtered.length-1?`1px solid ${T.border}`:"none",transition:"background .12s",gap:8}}
                  onMouseEnter={e=>e.currentTarget.style.background=T.surf2}
                  onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  ...
                  <select value={u.plan} onChange={e=>{setUsers(p=>p.map(x=>x.id===u.id?{...x,plan:e.target.value}:x));notify(`Plan changé → ${e.target.value}`,"success");}} style={{padding:"3px 6px",borderRadius:6,background:`${planColor[u.plan]||T.muted}15`,border:`1px solid ${planColor[u.plan]||T.muted}30`,color:planColor[u.plan]||T.muted,fontSize:11,fontWeight:700,cursor:"pointer"}}>
                    {["Free","Basic","Expert","Pro"].map(p=><option key={p}>{p}</option>)}
                  </select>
                  <span style={{fontSize:11,fontFamily:"JetBrains Mono",color:u.credits===Infinity?T.green:u.credits<5?T.pink:T.text}}>{u.credits===Infinity?"∞":u.credits}</span>
                  <span style={{fontSize:11,color:T.muted}}>{u.videos}</span>
                  <select value={u.status} onChange={e=>{setUsers(p=>p.map(x=>x.id===u.id?{...x,status:e.target.value}:x));notify(`Statut → ${e.target.value}`,"success");}} style={{padding:"3px 6px",borderRadius:6,background:`${statusColor[u.status]}15`,border:`1px solid ${statusColor[u.status]}30`,color:statusColor[u.status],fontSize:11,fontWeight:700,cursor:"pointer"}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    ...
                    <button onClick={()=>{if(window.confirm(`Supprimer ${u.name} ?`))setUsers(p=>p.filter(x=>x.id!==u.id));}} style={{padding:"4px 8px",borderRadius:6,background:`${T.pink}15`,border:`1px solid ${T.pink}30`,color:T.pink,fontSize:10,cursor:"pointer",fontWeight:600}} title="Supprimer">🗑</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <Modal title="✏️ Modifier l'utilisateur" onClose={()=>setEditUser(null)} width={440}>
          <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div>
                <label style={{fontSize:12,color:T.muted,display:"block",marginBottom:6,fontWeight:600}}>Crédits</label>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            ...
            <Btn onClick={()=>{setUsers(p=>p.map(u=>u.id===editUser.id?editUser:u));setEditUser(null);notify("Utilisateur mis à jour !","success");}}>Sauvegarder</Btn>
          </div>
        </Modal>
      )}
...
const NotFound=({onHome})=>(
  <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",textAlign:"center",padding:24}}>
    <div style={{fontSize:80,marginBottom:20,animation:"float 3s ease infinite"}}>🌌</div>
    <div style={{fontFamily:"JetBrains Mono",fontSize:80,fontWeight:900,color:T.dim,lineHeight:1}}>404</div>
    <div style={{fontWeight:800,fontSize:24,marginBottom:12,marginTop:8}}>Page introuvable</div>
    <div style={{color:T.muted,fontSize:15,marginBottom:32}}>Cette page n'existe pas ou a été supprimée.</div>
    <Btn onClick={onHome}>← Retour à l'accueil</Btn>
  </div>
);
...
const CheckoutPage=({plan,yearly,onBack,onSuccess,user})=>{
  ...
  const COUPONS={BETA50:50,WELCOME20:20,CREATOR30:30,SUMMER25:25};
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{background:T.surf,borderRadius:16,border:`1px solid ${T.border}`,padding:"20px 28px",maxWidth:340,margin:"0 auto 32px",textAlign:"left"}}>
          ...
          {[
            ...
            ["Crédits",selectedPlan.credits===Infinity?"Illimité":`${selectedPlan.credits}/mois`],
        ...
        <div>
          ...
          <div style={{marginBottom:28}}>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <Btn v={couponApplied?"success":"dark"} onClick={applyCoupon} disabled={!!couponApplied}>
                {couponApplied?`-${couponApplied}% ✓`:"Appliquer"}
              </Btn>
            </div>
          ...
          <Btn full size="lg" onClick={pay} style={{background:T.grad,animation:"glow 3s infinite",fontSize:16,padding:"16px 24px"}}>
            🔒 Payer ${finalPrice.toFixed(2)} / mois →
          </Btn>
...
const SubscriptionPage=({user,setUser,onBack})=>{
  ...
  const invoices=[
    {id:"INV-2026-003",date:"01/03/2026",amount:"$25.20",status:"Payée",plan:"Pro Mensuel"},
    {id:"INV-2026-002",date:"01/02/2026",amount:"$25.20",status:"Payée",plan:"Pro Mensuel"},
    {id:"INV-2026-001",date:"01/01/2026",amount:"$15.60",status:"Payée",plan:"Expert Mensuel"},
    {id:"INV-2025-012",date:"01/12/2025",amount:"$15.60",status:"Payée",plan:"Expert Mensuel"},
  ];
        ...
        {tab==="overview"&&(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:24}} className="mobile-grid1">
              {[
                {label:"Vidéos ce mois",used:usage.videos.used,total:usage.videos.total,color:T.acc,icon:"🎬"},
        ...
        {tab==="change"&&(
          <div>
              ...
              {PLANS.map(plan=>{
                ...
                return(
                  <div key={plan.id} style={{borderRadius:16,padding:"22px 18px",background:isActive?`${T.acc}10`:T.surf,border:`2px solid ${isActive?T.acc:T.border}`,position:"relative",transition:"all .2s",cursor:isActive?"default":"pointer"}}
                    ...
                    <div style={{fontFamily:"JetBrains Mono",fontWeight:900,fontSize:28,color:isActive?T.acc:T.text,marginBottom:4}}>${plan.priceY}<span style={{fontSize:12,fontWeight:400,color:T.muted}}>/mo</span></div>
                    <div style={{fontSize:11,color:T.muted,marginBottom:16}}>{plan.credits===Infinity?"Illimité":`${plan.credits} vidéos/mois`}</div>
                    {!isActive&&(
                      <Btn full size="sm" v={isUpgrade?"primary":"secondary"} onClick={()=>{notify(isUpgrade?`Passage au plan ${plan.name} 🚀`:`Rétrogradation vers ${plan.name}`,"success");setUser(u=>({...u,plan:plan.name}));}}>
            ...
            <div style={{marginTop:18,padding:"12px 16px",borderRadius:10,background:`${T.acc}08`,border:`1px solid ${T.acc}18`,fontSize:12,color:T.muted}}>
              ℹ️ Les changements de plan prennent effet immédiatement. Les crédits non utilisés sont remboursés au prorata sur ta prochaine facture.
            </div>
          </div>
        )}
            ...
            <>
              <div style={{padding:"16px",borderRadius:12,background:`${T.yellow}08`,border:`1px solid ${T.yellow}20`,marginBottom:20}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:5}}>
                  {["Accès aux {n} crédits restants ce mois".replace("{n}","7"),"Tous tes projets en cours","Les styles et templates sauvegardés","L'accès prioritaire au support"].map(l=>(
            ...
            <>
              <div style={{textAlign:"center",padding:"10px 0 20px"}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  <Btn v="danger" onClick={()=>{setCancelFlow(false);setCancelStep(0);setUser(u=>({...u,plan:"Free"}));notify("Abonnement annulé. À bientôt 😢","warning");}} full>Confirmer l'annulation</Btn>
                </div>
              </div>
            </>
          )}
  ...
  const pages={
    cgu:{
      ...
      sections:[
        ...
        {title:"10. Contact",content:"Pour toute question relative aux présentes CGU : legal@subcraftai.com · SubCraft SAS · 12 rue des Créateurs · 75001 Paris · France"},
      ]
    },
    privacy:{
      title:"Politique de Confidentialité",emoji:"🛡️",
      sections:[
        {title:"1. Responsable du traitement",content:"SubCraft SAS, 12 rue des Créateurs, 75001 Paris. DPO : privacy@subcraftai.com"},
    ...
    legal:{
      ...
      sections:[
        {title:"Éditeur du site",content:"SubCraft SAS · Société par Actions Simplifiée au capital de 10 000€ · RCS Paris 123 456 789 · Code APE : 6201Z · N° TVA intracommunautaire : FR12 123456789 · Siège social : 12 rue des Créateurs, 75001 Paris, France"},
        ...
        {title:"Propriété intellectuelle",content:"L'ensemble du contenu de ce site (textes, images, logos, code source) est protégé par le droit d'auteur et appartient à SubCraft SAS ou à ses partenaires. Toute reproduction sans autorisation est interdite."},
        {title:"Crédits",content:"Technologies utilisées : React (Meta), Whisper AI (OpenAI), Claude AI (Anthropic), Stripe (paiements), Cloudflare R2 (stockage)."},
      ]
    },
    rgpd:{
      ...
      sections:[
        ...
        {title:"Exercer vos droits",content:"Par email : privacy@subcraftai.com (réponse sous 30 jours) · Par courrier : SubCraft SAS · DPO · 12 rue des Créateurs · 75001 Paris · Vous pouvez également introduire une réclamation auprès de la CNIL : cnil.fr"},
      ]
    },
  };
...
const StatusPage=({onBack})=>{
  const services=[
    {name:"API Transcription (Whisper)",status:"operational",uptime:"99.98%",ping:"142ms"},
    {name:"Génération de sous-titres",status:"operational",uptime:"99.95%",ping:"89ms"},
    {name:"Claude AI (Emojis / Traduction)",status:"operational",uptime:"99.91%",ping:"210ms"},
    {name:"Export vidéo (FFmpeg)",status:"degraded",uptime:"98.20%",ping:"520ms"},
    {name:"Authentification",status:"operational",uptime:"100%",ping:"45ms"},
    {name:"Stockage Cloudflare R2",status:"operational",uptime:"99.99%",ping:"67ms"},
    {name:"Paiements Stripe",status:"operational",uptime:"99.99%",ping:"92ms"},
    {name:"Interface web",status:"operational",uptime:"99.96%",ping:"38ms"},
  ];
  const incidents=[
    {date:"5 Mars 2026",title:"Latence élevée sur l'export vidéo",severity:"warning",resolved:false,desc:"L'export vidéo avec sous-titres incrustés connaît des délais plus élevés que d'habitude. Notre équipe travaille à résoudre le problème. Export SRT/ASS non affecté."},
    {date:"28 Fév 2026",title:"Indisponibilité partielle de l'API Claude",severity:"incident",resolved:true,desc:"L'intégration Claude AI a été indisponible pendant 23 minutes entre 14h32 et 14h55 UTC. Résolu. Les fonctions d'emojis et traduction ont été rétablies."},
    {date:"15 Fév 2026",title:"Maintenance planifiée infrastructure",severity:"maintenance",resolved:true,desc:"Migration des serveurs vers une nouvelle région EU Frankfurt terminée avec succès. Amélioration des latences de 35%."},
  ];
  const statusConfig={operational:{label:"Opérationnel",color:T.green,dot:"🟢"},degraded:{label:"Dégradé",color:T.orange,dot:"🟡"},down:{label:"Indisponible",color:T.pink,dot:"🔴"}};
    ...
    <div style={{minHeight:"100vh",background:T.bg,padding:"0 24px 80px"}} className="page">
      <div style={{maxWidth:760,margin:"0 auto"}}>
        <div style={{padding:"22px 0 28px",display:"flex",alignItems:"center",gap:12}}>
          <Btn v="ghost" onClick={onBack}>← Retour</Btn>
          <h1 style={{fontWeight:800,fontSize:22}}>⚡ Status des services</h1>
        </div>
        {/* Global status */}
        <div style={{padding:"22px 28px",borderRadius:18,background:allOk?`${T.green}0a`:`${T.orange}0a`,border:`2px solid ${allOk?T.green:T.orange}33`,marginBottom:28,display:"flex",alignItems:"center",gap:16}}>
          <div style={{width:52,height:52,borderRadius:"50%",background:allOk?`${T.green}18`:`${T.orange}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{allOk?"✓":"⚠"}</div>
          <div>
            <div style={{fontWeight:800,fontSize:18,color:allOk?T.green:T.orange}}>{allOk?"Tous les systèmes opérationnels":"Dégradation partielle en cours"}</div>
            <div style={{color:T.muted,fontSize:13,marginTop:2}}>Mis à jour le {new Date().toLocaleDateString("fr-FR")} à {new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}</div>
          </div>
          <div style={{marginLeft:"auto",textAlign:"right"}} className="hide-mobile">
            <div style={{fontFamily:"JetBrains Mono",fontWeight:800,fontSize:22,color:allOk?T.green:T.orange}}>99.95%</div>
            <div style={{fontSize:11,color:T.muted}}>uptime 90 jours</div>
          </div>
        </div>
        {/* Services */}
        <div style={{background:T.surf,borderRadius:16,border:`1px solid ${T.border}`,overflow:"hidden",marginBottom:24}}>
          <div style={{padding:"13px 20px",borderBottom:`1px solid ${T.border}`,fontWeight:700,fontSize:14}}>Services</div>
          {services.map((s,i)=>{
            const cfg=statusConfig[s.status];
            return(
              <div key={s.name} style={{padding:"13px 20px",borderBottom:i<services.length-1?`1px solid ${T.border}`:"none",display:"flex",alignItems:"center",gap:12,transition:"background .12s"}} onMouseEnter={e=>e.currentTarget.style.background=T.surf2} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:500}}>{s.name}</div>
                </div>
                <div style={{display:"flex",gap:16,alignItems:"center"}} className="hide-mobile">
                  <span style={{fontSize:11,color:T.muted,fontFamily:"JetBrains Mono"}}>↑ {s.uptime}</span>
                  <span style={{fontSize:11,color:T.muted,fontFamily:"JetBrains Mono"}}>⚡ {s.ping}</span>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 10px",borderRadius:100,background:`${cfg.color}12`,border:`1px solid ${cfg.color}22`}}>
                  <span style={{width:7,height:7,borderRadius:"50%",background:cfg.color,animation:s.status!=="down"?"pulseDot 2s infinite":undefined}}/>
        ...
        {/* Uptime sparkline 90j */}
        <div style={{background:T.surf,borderRadius:14,border:`1px solid ${T.border}`,padding:"16px 18px",marginBottom:20}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:12}}>📊 Uptime 90 jours</div>
          <div style={{display:"flex",gap:2,alignItems:"flex-end",height:36}}>
            {Array.from({length:45}).map((_,i)=>{
              const h=Math.random()<0.97?80+Math.random()*20:30+Math.random()*40;
              const color=h>90?T.green:h>60?T.yellow:T.pink;
              return <div key={i} style={{flex:1,height:h+"%",background:color,borderRadius:"2px 2px 0 0",minWidth:3,cursor:"default"}} title={"Jour "+(i+1)+" — "+h.toFixed(1)+"%"}/>;
            })}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:10,color:T.dim}}>
            <span>90 jours</span>
            <span style={{color:T.green,fontWeight:700}}>Aujourd'hui 99.95%</span>
          </div>
        </div>

        {/* Incidents */}
        <div style={{background:T.surf,borderRadius:16,border:`1px solid ${T.border}`,overflow:"hidden"}}>
          <div style={{padding:"13px 20px",borderBottom:`1px solid ${T.border}`,fontWeight:700,fontSize:14}}>Incidents récents</div>
          {incidents.map((inc,i)=>{
            const sev={warning:{color:T.orange,icon:"⚠️"},incident:{color:T.pink,icon:"🔴"},maintenance:{color:T.cyan,icon:"🔧"}};
            const s=sev[inc.severity];
            return(
              <div key={inc.title} style={{padding:"16px 20px",borderBottom:i<incidents.length-1?`1px solid ${T.border}`:"none"}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                  <span style={{fontSize:16,flexShrink:0,marginTop:1}}>{s.icon}</span>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:5}}>
                      <span style={{fontWeight:700,fontSize:13}}>{inc.title}</span>
                      {inc.resolved?<Tag color={T.green}>Résolu</Tag>:<Tag color={s.color} dot>En cours</Tag>}
                    </div>
                    <div style={{fontSize:11,color:T.muted,marginBottom:6}}>{inc.date}</div>
                    <div style={{fontSize:12,color:T.muted,lineHeight:1.6}}>{inc.desc}</div>
                  </div>
                </div>
              </div>
            );
...
const ReferralPage=({user,onBack})=>{
  ...
  const refCode=user?.email?.split("@")[0]?.toUpperCase().slice(0,8)||"CREATOR";
  const refLink=`https://subcraftai.com/ref/${refCode}`;
      ...
      <div style={{maxWidth:760,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:18,flexWrap:"wrap"}}>
          {[
            ...
            {icon:"⚡",val:"Instantané",label:"crédit appliqué auto",color:T.purple},
          ].map(s=>(
        ...
        {tab==="share"&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {/* Share buttons */}
            <div style={{background:T.surf,borderRadius:14,border:`1px solid ${T.border}`,padding:20}}>
              <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>📱 Partager directement</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:8}}>
                {[["WhatsApp","#25d366","💬"],["Twitter/X","#1da1f2","🐦"],["Instagram","#e1306c","📸"],["LinkedIn","#0077b5","💼"],["Email","#666","📧"],["Copier SMS","#555","📱"]].map(([pl,color,ic])=>(
                  <button key={pl} onClick={()=>{notify(`Ouverture de ${pl}...`,"info");copyLink();}} style={{padding:"10px 12px",borderRadius:10,background:`${color}15`,border:`1px solid ${color}25`,color,fontWeight:600,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6,cursor:"pointer",transition:"all .15s"}} onMouseEnter={e=>e.currentTarget.style.background=`${color}25`} onMouseLeave={e=>e.currentTarget.style.background=`${color}15`}>
                    {ic} {pl}
                  </button>
                ))}
              </div>
...
**1. Upload ta première vidéo** → 3 crédits gratuits t'attendent
...
Sophie`
    },
    {
      ...
      subject:"Tes 3 crédits gratuits expirent bientôt ⚠️",
      from:"Sophie de SubCraft <hello@subcraftai.com>",
      body:`Salut {{prénom}},
...
🎁 1 mois de crédits bonus (déjà ajoutés à ton compte)
...
export default function App(){
  ...
  const goCheckout=(planId,yearly=true)=>{
    if(!user){nav("login");return;}
    setCheckoutPlan(planId||"pro");
  ...
  return(
    <>
      ...
      {user&&!pushDismissed&&(
        <div style={{position:"fixed",bottom:76,left:"50%",transform:"translateX(-50%)",zIndex:400,background:T.surf2,border:`1px solid ${T.acc}40`,borderRadius:16,padding:"12px 18px",display:"flex",gap:12,alignItems:"center",boxShadow:"0 8px 40px rgba(0,0,0,.6)",width:"min(440px,90vw)",animation:"popIn .4s cubic-bezier(.34,1.56,.64,1)"}}>
          <span style={{fontSize:24,flexShrink:0}}>🔔</span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontWeight:700,fontSize:13,marginBottom:2}}>Activer les notifications push</div>
            <div style={{fontSize:11,color:T.muted}}>Sois alerté dès que ta vidéo est prête — sans rester sur la page !</div>
          </div>
          <div style={{display:"flex",gap:6,flexShrink:0}}>
            <button onClick={()=>setPushDismissed(true)} style={{padding:"5px 10px",borderRadius:7,background:"transparent",border:`1px solid ${T.border}`,color:T.muted,fontSize:11,cursor:"pointer"}}>Plus tard</button>
            <button onClick={()=>{setPushDismissed(true);notify("🔔 Notifications activées !","success");showPushToast("SubCraft","Notifications activées ! Tu seras alerté quand tes vidéos sont prêtes 🎉");}} style={{padding:"5px 12px",borderRadius:7,background:T.acc,border:"none",color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer"}}>Activer</button>
          </div>
        </div>
      )}

      {page==="landing"&&<LandingPage onCTA={()=>nav("signup")} setPage={nav}/>}
      ...
      {page==="dashboard"&&<Dashboard user={user} setUser={setUser} onOpen={f=>{setCurrentFile(f);nav("editor");}} onLogout={()=>{setUser(null);nav("landing");}} setPage={nav}/>}
      {page==="editor"&&<EditorPage onBack={()=>nav("dashboard")} file={currentFile}/>}
      {page==="pricing"&&<PricingPage onBack={()=>nav(user?"dashboard":"landing")} onSelect={(planId)=>goCheckout(planId,true)}/>}
      ...
      {cookieConsent===null&&!["checkout","payment-failed",...LEGAL_PAGES].includes(page)&&(
        <CookieBanner onAccept={(prefs)=>setCookieConsent(prefs)} onDecline={()=>setCookieConsent({necessary:true,analytics:false,marketing:false})} setPage={nav}/>
      )}
    </>
  );
}
