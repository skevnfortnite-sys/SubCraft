...
const LoginPage=({onAuth,onAdmin,goSignup})=>{
  ...
  const handle=()=>{
    ...
    if(btoa(email+":"+pw)===_adm){onAdmin();return;}
    setLoading(true);
    setTimeout(()=>{
      setLoading(false);
      // Simulate wrong password for demo (in prod → server auth)
      if(pw.length>=6){
        onAuth({name:email.split("@")[0],email,plan:"Free",isNew:false});
      } else {
        setAttempts(a=>a+1);
        setErr(`Identifiants incorrects. ${LOCKOUT-(attempts+1)} essai(s) restant(s).`);
      }
    },1200);
  };
  return(
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",background:"#010208",position:"relative"}}>
      ...
      <ConfirmModal
        ...
        onConfirm={()=>{setUsers(p=>p.filter(x=>x.id!==confirmDeleteUser.id));setConfirmDeleteUser(null);notify("Utilisateur supprimé","warning");}}
